...
const registerSchema = object({
  ...
  nb_stud: string().min(1,"Le nombre d'étudiants est obligatoire"),
...
const fields = [
  ...
  { field: "nb_stud", headerName: "N° d'étudiants", type: "number", add: true, edit: true, required: true},
  {
    field: "active",
    headerName: "Etat",
    type: "checkbox",
...
const NewClass: React.FC<NewItemProps> = ({ open, handleClose, handleCloseUpdated, handleRefresh, item, setItem }) => {
  ...
  const onSubmitHandler: SubmitHandler<RegisterInput> = (values: any) => {
    const parsedValues = { ...values, nb_stud: Number(values.nb_stud) };
